...
class TestQueen(unittest.TestCase):
    ...
    def test_can_draw_image(self):
        ...
        Queen().image(HexMeta(50), PieceColor.White())

    def test_basic_moves(self):
        """We test the basic case where there are no other pieces on the board except a single queen."""
        board: Board = Board(4)
        queen_coord: BoardCoordinate = BoardCoordinate(3, -3)
        board_state: BoardState = (
            BoardState.builder(board)
            .add_piece(Player.WHITE, queen_coord, PieceType.QUEEN)
            .build())

        # We want to test we can get the appropriate Moves class so we get that from the piece on the board itself.
        piece_info = board_state.get_piece_info(queen_coord)
        self.assertEqual(piece_info.player, Player.WHITE)
        self.assertEqual(piece_info.piece_type, PieceType.QUEEN)

        moves = get_piece_from_type(piece_info.piece_type).moves(queen_coord, board_state)
        expected_moves = set((
            BoardCoordinate(0, -3),
            BoardCoordinate(1, -3),
            BoardCoordinate(2, -3),
    ...
    def test_moving_piece_that_doesnt_exist_fails(self):
        ...
        board: Board = Board(7)
        queen_coord: BoardCoordinate = BoardCoordinate(1, -4)
        board_state: BoardState = (
            BoardStateBuilder(board)
            .add_piece(Player.WHITE, queen_coord, PieceType.QUEEN)
            .build())
        ...
        with self.assertRaises(Exception):
            Queen().moves(BoardCoordinate(0, 0), board_state)

    def test_moving_non_queen_fails(self):
        """If we try to get moves for a coordinate that is not a queen we should fail."""
        board: Board = Board(7)
        king_coord: BoardCoordinate = BoardCoordinate(1, -4)
        queen_coord: BoardCoordinate = BoardCoordinate(0, -4)
        board_state: BoardState = (
            BoardStateBuilder(board)
